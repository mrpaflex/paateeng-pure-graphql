...
export class UserService {
  ...
  async createuser(createUserInput: CreateUserInput): Promise<returnString> {
    try {
  ...
  async changeUserPassword(
    ...
    try {
      ...
      if (
        userChangepassword.password !== userChangepassword.confirmedPassword
      ) {
        throw new ConflictException(
          'password and confirmed password do not matched',
        );
      }

      if (
        (await comparePassword(
          userChangepassword.confirmedOldPassword,
          user.password,
        )) === false
      ) {
        throw new ConflictException(
          'your confirmed old password does not matched',
        );
      }
